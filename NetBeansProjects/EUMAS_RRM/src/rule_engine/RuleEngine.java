...
public class RuleEngine {
    ...
    private final HashMap<Integer, HashMap<HashSet<String>, HashMap<String, Boolean>>> strict_rules;
//    private final HashMap <Integer,HashMap<HashSet<String>,HashMap<String,Boolean>>> complex_rules;
    ...
    private final HashMap<Integer, HashMap<String, Boolean>> strict_facts;
    ...
    public RuleEngine() {
        ...
        this.strict_facts = new HashMap<>();
        this.strict_rules = new HashMap<>();
...
//    }
    public RuleEngine(RuleEngine KB) {
        ...
        this.strict_rules = (HashMap<Integer, HashMap<HashSet<String>, HashMap<String, Boolean>>>) this.deepClone(KB.strict_rules);
        this.strict_facts = (HashMap<Integer, HashMap<String, Boolean>>) this.deepClone(KB.strict_facts);
//        this.complex_rules= (HashMap<Integer,HashMap<HashSet<String>,HashMap<String,Boolean>>>)this.deepClone(KB.complex_rules);
    ...
    public void print_strict_facts() {
        System.out.println("strict facts are : ");
        System.out.println(strict_facts);
    }

    public void print_strict_rules() {
        System.out.println("strict rules are : ");
        System.out.println(strict_rules);
    }
    ...
    private void readRules(String rule) {
        ...
        if (st.countTokens() == 2) {
            ...
            if (a.equals("->")) {
                ...
                strict_facts.put(this.getTotalRules(), list);
            ...
            if (b.equals("=>")) {
                ...
                list.put(c, true);
                list1.put(set, list);
                strict_rules.put(this.getTotalRules(), list1);
                this.increaseTotalRules();
...
//                        this.reasoner();
        ...
        for (Integer count : strict_rules.keySet()) {
            for (HashSet s : strict_rules.get(count).keySet()) {
                if (inferted.containsAll(s)) {

                    for (String value : strict_rules.get(count).get(s).keySet()) {
                        if (!inferted.contains(value)) {
                            inferted.add(value);
                            this.reasoner();
                        }
...
//                        set.add(str1);
        ...
        for (Integer str : strict_facts.keySet()) {
            //vale mesa ola ta facts
//            System.out.println(facts.get(str));
            for (String str2 : strict_facts.get(str).keySet()) {
                if (strict_facts.get(str).get(str2)) {

                    if (inferted.isEmpty() || !inferted.contains(str2)) {
                        for (String str1 : strict_facts.get(str).keySet()) {
                            this.inferted.add(str1);
//                        HashSet<String> set= new HashSet<>();
//                        set.add(str1);
                            this.reasoner();
...
//                    System.out.println("to fact poy mpike :"+fact);
    ...
    public List<Integer> returnInActiveRules() {
        List<Integer> IDset = new ArrayList<>();
        for (Integer rule : rules.keySet()) {
            for (HashSet str : rules.get(rule).keySet()) {
                for (String str1 : rules.get(rule).get(str).keySet()) {
...
//                    System.out.println("to fact poy mpike :"+fact);
    ...
    /**
     * returns a preference with ID if the ID is not in the Preferences returns
     * NULL
     *
     * @param ID
     * @return
